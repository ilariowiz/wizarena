...
export const setNetworkSettings = (
  netId,
  chainId,
...
export const connectXWallet = (
  ...
  return async (dispatch) => {
    ...
    connectlabel: try {
      ...
      res = await window.kadena.request({
        method: "kda_connect",
        networkId: netId,
      });
...
export const connectChainweaver = (
  account,
  chainId,
  gasPrice,
  gasLimit,
  ...
  return async (dispatch) => {
    ...
    if (
      parsedLocalRes &&
      parsedLocalRes.result &&
      parsedLocalRes.result.status === "success"
    ) {
      dispatch(
        fetchAccountDetails(
          account,
  ...
  return async (dispatch) => {
    const signClient = await SignClient.init({
      ...
      metadata: {
        ...
        icons: [
          "https://firebasestorage.googleapis.com/v0/b/raritysniperkda.appspot.com/o/android-chrome-384x384.png?alt=media&token=e5946e6e-ac87-446a-91f4-f6144906ef22",
        ],
      },
    });
      ...
      const accounts = session.namespaces["kadena"].accounts.map((item) => {
        ...
        ["mainnet01"].forEach((chain) => {
          if (wallet && wallet.includes("Koala")) {
            normalAccountName = normalAccountName
              .replace("**", ":")
              .replace(`kadena:${chain}:`, "");
          } else {
            normalAccountName = `k:${normalAccountName.replace(
              `kadena:${chain}:`,
              ""
            )}`;
          }
...
export const connectLinx = (
  ...
  return async (dispatch) => {
    ...
    try {
      if (linx) {
        const account = await linx(
          linxNewRequest("Account", "get address", {}, false)
        );
        if (account) {
          dispatch(
            fetchAccountDetails(
          ...
          callback(null, account); // Pass the account to the callback
        } else {
          throw new Error("Linx account not found");
        }
      } else {
        throw new Error("Linx not found");
      }
    } catch (error) {
  ...
  return (dispatch) => {
    ...
    dispatch(readFromContract(cmd, true, networkUrl)).then((response) => {
      ...
      if (response) {
        ...
        dispatch({
          type: LOAD_ALL_NFTS_IDS,
          payload: { allNftsIds: response },
        });
        ...
        partsBlock.map((pr) => {
          let promise = Promise.resolve(
            dispatch(
              loadBlockNftsSplit(chainId, gasPrice, 180000, networkUrl, pr)
            )
          );
        ...
        Promise.all(promises).then((values) => {
          ...
          final.map((i) => {
            ...
            if (i.attack.int > maxStats.attack) {
              maxStats["attack"] =
                i.attack.int + i["upgrades-spell"].attack.int;
            }
            if (i.damage.int > maxStats.damage) {
              maxStats["damage"] =
                i.damage.int + i["upgrades-spell"].damage.int;
            }
...
export const loadBlockNftsSplit = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  networkUrl,
...
export const loadBlockNftsSubscribed = (
  response,
  tournamentType,
  callbackSubscribed
) => {
  return (dispatch) => {
    ...
    if (tournamentType === "kda") {
      dispatch({
        ...
        payload: {
          nfts: newResponse,
          subscribedKdaSpellGraph: subscribedSpellGraph,
        },
      });
    } else if (tournamentType === "wiza") {
      dispatch({
        type: LOAD_SUBSCRIBED_WIZA,
        payload: {
          nfts: newResponse,
          subscribedWizaSpellGraph: subscribedSpellGraph,
        },
      });
    } else if (tournamentType === "elite") {
      dispatch({
        type: LOAD_SUBSCRIBED_ELITE,
        payload: {
          nfts: newResponse,
          subscribedEliteSpellGraph: subscribedSpellGraph,
        },
      });
    }
  };
    ...
    dispatch(readFromContract(cmd, true, networkUrl)).then((response) => {
      ...
      if (response && response.length > 0) {
        ...
        partsBlock.map((pr) => {
          let promise = Promise.resolve(
            dispatch(
              loadBlockItemsSplit(chainId, gasPrice, 175000, networkUrl, pr)
            )
          );
          promises.push(promise);
        });
...
export const loadBlockItemsSplit = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  networkUrl,
  ...
  return async (dispatch) => {
    let cmd = {
      pactCode: `(free.${CONTRACT_NAME_EQUIPMENT}.get-equipment-fields-for-ids ${JSON.stringify(
        block
      )})`,
      meta: defaultMeta(chainId, gasPrice, gasLimit),
    };
...
export const loadBurningNftInfo = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  networkUrl,
...
export const readAccountMinted = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 300,
  networkUrl,
...
export const loadUserMintedNfts = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  networkUrl,
...
export const getCollectionOffers = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 60000,
  networkUrl,
...
export const getCollectionOffersForBuyer = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 60000,
  networkUrl,
...
export const getEquipmentActiveOffers = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 60000,
  networkUrl,
...
export const getEquipmentOffersMade = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 60000,
  networkUrl,
...
export const loadMaxItemsPerWallet = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 300,
  networkUrl,
...
export const getPotionEquippedMass = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 30000,
  networkUrl,
...
export const getPotionEquipped = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 300,
  networkUrl,
...
export const getPvPsubscription = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 150000,
  networkUrl,
  ...
  return (dispatch) => {
    ...
    let cmd = {
      pactCode: `(free.${CONTRACT_NAME}.get-pvp-subscriptions ${JSON.stringify(
        ids
      )})`,
      meta: defaultMeta(chainId, gasPrice, gasLimit),
    };
...
export const getConquestSubscribersIdsPerSeason = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 180000,
  networkUrl,
...
export const getDungeonSubscribersIdsPerSeason = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 180000,
  networkUrl,
...
export const getSubscriptions = (
  ...
  return (dispatch) => {
    let cmd = {
      pactCode: `(free.${CONTRACT_NAME}.get-subscriptions ${JSON.stringify(
        ids
      )})`,
      meta: defaultMeta(chainId, gasPrice, gasLimit),
    };
...
export const getCountForTournament = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 10000,
  networkUrl,
...
export const getInfoNftBurning = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 300,
  networkUrl,
...
export const checkAddressForPrice = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 300,
  networkUrl,
...
export const addNftToBurningQueue = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  netId,
  idnft,
...
export const removeNftFromBurningQueue = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  netId,
  idnft,
...
export const forgeItem = (
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME_EQUIPMENT}.forge "${recipe}" ${JSON.stringify(
      ingredients
    )} "${account.account}" free.wiza)`;
...
export const makeOffer = (
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.make-offer "${idNft}" "${
      account.account
    }" ${days} ${_.round(amount).toFixed(1)})`;
...
export const makeCollectionOffer = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.make-collection-offer "${
      account.account
    }" ${days} ${_.round(amount).toFixed(1)})`;
...
export const acceptCollectionOffer = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
...
export const withdrawCollectionOffer = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    ...
    let caps = [
      Pact.lang.mkCap(
        ...
        "Withdraw Collection offer",
        `coin.TRANSFER`,
        ["wizards-offers-bank", account.account, _.round(amount, 2)]
      ),
      Pact.lang.mkCap(
        "Verify owner",
        "Verify your are the owner",
...
export const makeOfferEquipment = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME_EQUIPMENT}.make-offer "${
      account.account
    }" "${ringType}" ${days} ${_.round(amount).toFixed(1)} free.wiza)`;
...
export const acceptOfferEquipment = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
...
export const withdrawEquipmentOffer = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    ...
    let caps = [
      Pact.lang.mkCap(
        ...
        "Withdraw offer",
        `free.wiza.TRANSFER`,
        ["wiz-equipment-offers-bank", account.account, _.round(amount, 2)]
      ),
      Pact.lang.mkCap(
        "Verify owner",
        "Verify your are the owner",
...
export const subscribeToTournamentMass = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-tournament-mass ${JSON.stringify(
      list
    )} "${account.account}")`;
...
export const subscribeToTournamentMassWIZA = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-tournament-wiza-mass ${JSON.stringify(
      list
    )} "${account.account}" free.wiza)`;
...
export const subscribeToTournamentMassELITE = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-tournament-elite-mass ${JSON.stringify(
      list
    )} "${account.account}" free.wiza)`;
...
export const subscribeToLords = (
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-to-lord-season-mass "${
      account.account
    }" "${seasonId}" ${JSON.stringify(list)})`;
...
export const subscribeToDungeons = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-to-dungeon-season-mass "${
      account.account
    }" "${seasonId}" ${JSON.stringify(list)})`;
...
export const subscribeToPvPMass = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.subscribe-pvp-mass ${JSON.stringify(
      list
    )} "${account.account}" free.wiza)`;
...
export const incrementFightPvP = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    ...
    let caps = [
      ...
      Pact.lang.mkCap(
        `Increment PvP fights`,
        "Pay to increment fights",
        `free.wiza.TRANSFER`,
        [account.account, WIZA_TOKEN_BANK, _.round(wizaAmount, 2)]
      ),
      Pact.lang.mkCap("Gas capability", "Pay gas", "coin.GAS", []),
    ];
...
export const changeSpellTournament = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit,
  netId,
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.change-spell-tournament "${idNft}" "${
      account.account
    }" ${JSON.stringify(spellSelected)})`;
...
export const resetSpellUpgrades = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  netId,
  account,
...
export const getAuraUpgradeCost = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 50000,
  networkUrl,
...
export const swapKdaWiza = (
  ...
  return (dispatch) => {
    ...
    let slippage = 0.05;
    let token0AmountWithSplippage =
      parseFloat(amountKda) + (parseFloat(amountKda) * slippage) / 100;
...
export const sendChallenge = (
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.send-challenge "${nft1id}" "${nft2id}" ${_.round(
      amount
    ).toFixed(1)} "${coin}" free.wiza)`;
    ...
    if (coin === "kda") {
      caps.push(
        Pact.lang.mkCap(
          ...
          "Send challenge KDA",
          `coin.TRANSFER`,
          [account.account, "wizards-offers-bank", _.round(amount, 2)]
        )
      );
    } else {
      caps.push(
        Pact.lang.mkCap(
          ...
          "Send challenge WIZA",
          `free.wiza.TRANSFER`,
          [account.account, "wiz-auto-tournaments-bank", _.round(amount, 2)]
        )
      );
    }
    ...
    if (coin === "kda") {
      ...
      caps.push(
        Pact.lang.mkCap(
          ...
          "Cancel challenge",
          `free.wiza.TRANSFER`,
          ["wiz-auto-tournaments-bank", account.account, _.round(amount, 2)]
        )
      );
    }
...
export const getChallengesSent = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 75000,
  networkUrl,
...
export const getChallengesReceived = (
  chainId,
  gasPrice = DEFAULT_GAS_PRICE,
  gasLimit = 75000,
  networkUrl,
...
export const createTournament = (
  ...
  return (dispatch) => {
    let pactCode = `(free.${CONTRACT_NAME}.create-tournament "${idnft}" "${
      account.account
    }" ${_.round(buyin).toFixed(
      1
    )} ${maxLevel} "${name}" ${winners} ${nPlayers} "${type}" free.wiza)`;
    ...
    let cmd = {
      ...
      caps: [
        ...
        Pact.lang.mkCap(
          ...
          `free.wiza.TRANSFER`,
          [account.account, "wiz-auto-tournaments-bank", _.round(buyin, 2)]
        ),
        Pact.lang.mkCap("Gas capability", "Pay gas", "coin.GAS", []),
      ],
      sender: account.account,
...
export const getTotalMined = (
  ...
  return (dispatch) => {
    ...
    dispatch(readFromContract(cmd, true, networkUrl)).then((response) => {
      ...
      if (response) {
        dispatch({
          type: STORE_TOTAL_MINED,
          payload: _.floor(response.decimal, 2),
        });
  ...
  return async (dispatch) => {
    ...
    if (linx) {
      const req = linxRequestData(
        cmdToSign, // signing command
        "Wizards Arena transaction", // Add any description here to make clear to user what the TX is for
        undefined, // Url of an image you want to show
        parseInt(chainId.toString()), // number
        "free.wiza", // tokenId : 'free.wiz' or 'coin'
        0.0, // amount
        0.0, // Amount if dapp adds any fee
        undefined, // fee token id, eg: 'coin' if dappfee is in kda
        false // chainless, not accessible atm so always false
      );

      const result = await linx(
        linxNewRequest(
          "Send",
          "Approve request for Wizards Arena transaction.",
          req,
          true
        )
      );
      ...
      if (result !== undefined && result.error === undefined) {
        ...
        dispatch(
          updateTransactionState(
            "error",
            "Failed to sign the command in Linx Wallet"
          )
        );
        return;
      }
      ...
      try {
        ...
        if (accountConnectedRes && accountConnectedRes.status !== "success") {
          ...
          dispatch(
            updateTransactionState(
              "error",
              "eckoWallet connection was lost, please re-connect"
            )
          );
          ...
          dispatch(
            updateTransactionState(
              "error",
              `Wrong eckoWallet account selected in extension, please select ${account.account}`
            )
          );
          return;
        }
      ...
      if (xwalletSignRes.status !== "success") {
        ...
        dispatch(
          updateTransactionState(
            "error",
            `Failed to sign the command in eckoWallet`
          )
        );
        return;
      }
      ...
      try {
        ...
        if (!signedCmd) {
          ...
          dispatch(
            updateTransactionState(
              "error",
              "Failed to sign the command in the wallet"
            )
          );
          return;
        }
    ...
    try {
      ...
      dispatch(
        updateTransactionState(
          "error",
          "Failed to confirm transaction with the network"
        )
      );
      return;
    }
    ...
    if (
      ...
      const loaderModalTransaction = document
        .querySelector("#loaderModalTransaction")
      ...
      try {
        ...
        dispatch(
          updateTransactionState(
            "error",
            "Had issues sending the transaction to the blockchain"
          )
        );
        return;
      }
      ...
      if (
        parsedLocalRes &&
        parsedLocalRes.result &&
        parsedLocalRes.result.status === "failure"
      ) {
        if (
          parsedLocalRes.result.error &&
          parsedLocalRes.result.error.message
        ) {
...
export const signFightTransaction = (
  gasPrice,
  chainId,
  netId,
  isXWallet,
  ...
  return async (dispatch) => {
    ...
    if (linx) {
      const req = linxRequestData(
        cmdToSign, // signing command
        "Sign the transaction to fight", // Add any description here to make clear to user what the TX is for
        undefined, // Url of an image you want to show
        parseInt(chainId.toString()), // number
        "free.wiza", // tokenId : 'free.wiz' or 'coin'
        0.0, // amount
...
const linxRequestData = function (
  signingRequest,
  itemDescription,
  imageUrl,
  chainId,
