...
export const linxRequestData = function (
  signingRequest,
  itemDescription,
  imageUrl,
  chainId,
...
export const signTransaction = (cmdToSign, isXWallet, isQRWalletConnect, isLinxWallet, qrWalletConnectClient, netId, networkUrl, account, chainId, nftId, callback) => {
	return async (dispatch) => {
...
export const signFightTransaction = (gasPrice, chainId, netId, isXWallet, isQRWalletConnect, isLinxWallet, qrWalletConnectClient, networkUrl, account, callback) => {
	return async (dispatch) => {
		...
		if (isXWallet) {
			...
			const req = linxRequestData(
				cmdToSign, // signing command
				"Wizards Arena Account Check", // Add any description here to make clear to user what the TX is for
				undefined, // Url of an image you want to show
				chainId, // number
				"free.wiza", // tokenId : 'free.wiz' or 'coin'
				0.0, // amount
			...
			const result = await linx(
				linxNewRequest(
					"Send",
					"Approve request for Wizards Arena account check.",
					req,
					true
				)
			);
			...
			if (result !== undefined && result.error === undefined) {
				...
				console.log("Failed to sign the command in Linx Wallet")
				// No error dispatch in this function as it uses a callback
			}
		}
		else if(isQRWalletConnect) {
