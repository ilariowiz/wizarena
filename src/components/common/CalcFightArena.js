...
import calcHitFight from './CalcHitFight'
...
const turno = (attaccante, difensore) => {
    ...
    if (skipTurn) {
        ...
        const colpito = calcHitFight(atkAttaccante, difesaDif, attaccante.bonusMiss || 0)

        if (!colpito) {
            if (attaccante['bonusMiss']) {
                attaccante['bonusMiss'] += 1
            }
            else {
                attaccante['bonusMiss'] = 1
            }
