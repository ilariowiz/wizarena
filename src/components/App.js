...
class App extends Component {
	...
	checkAndConnectLinx() {
		const checkLinxAvailability = setInterval(() => {
			if (window.flutter_inappwebview) {
				clearInterval(checkLinxAvailability)
				this.connectLinx()
			}
		}, 1000)
	}
	...
	connectLinx() {
		...
		this.props.connectLinx(chainId, gasPrice, gasLimit, networkUrl, (error, account) => {
			if (!error) {
				console.log(`Connected to Linx Wallet: ${account}`)
				this.refreshUser()
			} else {
				console.error("Failed to connect to Linx Wallet:", error)
			}
		})
	}
